...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $common_passwords = ['password123', 'admin123', 'kalika123', strtolower($auth_data['username'])];
        foreach ($common_passwords as $common) {
            if (stripos($new, $common) !== false) {
                throw new Exception("Password contains common or predictable terms.");
            }
        }
        ...
        if (file_put_contents(AUTH_FILE, json_encode($auth_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES))) {
            audit_log("password_changed", "Admin password successfully updated.");
        ...
        audit_log("password_change_failed", $error_msg);
    }
}
?>
<!DOCTYPE html>
